...
###############
  ...
  n=$( query 'SELECT * FROM T_Agglomerations GROUP BY aggCode HAVING COUNT(aggCode)>1;' --count);
  echo ">> Aglomeracions duplicades 'COUNT(aggCode)>1': $n"

  # aglomeracions amb longitud o latitud NULL
  n=$( query 'SELECT * FROM T_Agglomerations WHERE aggLongitude is NULL OR aggLatitude is NULL' --count);
...
#############
  ...
  n=$( query 'SELECT * FROM T_UWWTPS WHERE uwwLatitude is NULL OR uwwLongitude is NULL' --count);
...
##################
  ...
  n=$( query 'SELECT * FROM T_DischargePoints WHERE dcpLatitude is NULL OR dcpLongitude is NULL' --count);
...
#SELECT * FROM T_Agglomerations WHERE aggCode NOT IN (SELECT aucAggCode FROM T_UWWTPAgglos)
...
#SELECT * FROM T_UWWTPS WHERE uwwCode NOT IN (SELECT uwwCode FROM T_DischargePoints)
#SELECT * FROM T_UWWTPS WHERE uwwCode IN (SELECT uwwCode FROM T_DischargePoints GROUP BY uwwCode HAVING COUNT(uwwCode)>1)
#SELECT * FROM T_UWWTPS WHERE uwwCode NOT IN (SELECT aucUwwCode FROM T_UWWTPAgglos)
#SELECT * FROM T_DischargePoints GROUP BY dcpCode HAVING COUNT(dcpCode)>1
#SELECT * FROM T_DischargePoints WHERE dcpLatitude is NULL OR dcpLongitude is NULL
#SELECT * FROM T_DischargePoints WHERE uwwCode NOT IN (SELECT uwwCode FROM T_UWWTPS)
...
#SELECT * FROM T_UWWTPAgglos GROUP BY aucAggCode HAVING COUNT(aucAggCode)>1
#SELECT * FROM T_UWWTPAgglos GROUP BY aucUwwCode HAVING COUNT(aucUwwCode)>1
#SELECT * FROM T_Agglomerations AS agg, T_UWWTPS AS uww WHERE agg.aggCode = uww.aggCode
#SELECT * FROM T_UWWTPS AS uww, T_DischargePoints AS dcp WHERE uww.uwwCode = dcp.uwwCode
